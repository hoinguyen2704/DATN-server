...
@Service
public class GoogleLinkIntentTicketService {
    ...
    public TicketPayload consume(String ticket) {
        ...
        if (payload == null || payload.expiresAt().isBefore(LocalDateTime.now())) {
            throw new UnauthorizedException("Google link ticket is invalid or expired");
        }
    ...
    public record TicketPayload(UUID userId, LocalDateTime expiresAt) {
    }
}
