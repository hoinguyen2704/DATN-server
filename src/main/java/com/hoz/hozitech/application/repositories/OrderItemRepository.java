...
public interface OrderItemRepository extends JpaRepository<OrderItem, UUID> {
        ...
        @Query("SELECT oi.variant.id, oi.variant.product.id, oi.variant.product.name, oi.variant.variantName, " +
                        ...
                        +
                        "ORDER BY SUM(oi.quantity) DESC")
        List<Object[]> findTopSellingVariants(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to,
                        Pageable pageable);
        ...
        @Query("SELECT oi.variant.product.category.id, oi.variant.product.category.name, " +
                        ...
                        "ORDER BY SUM(oi.quantity) DESC")
        List<Object[]> findTopSellingCategories(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to,
                        Pageable pageable);
}
